...
testthat::test_that(
  ...
  testthat::expect_true(any(grepl("A_X", names(res))))
  testthat::expect_true(all(res$adm2_code == "11"))
})
...
testthat::test_that(
  ...
  testthat::expect_true(is.data.frame(res))
  testthat::expect_true(any(grepl("B_X", names(res))))
...
testthat::test_that(
  "anycensusk handles invalid year", {
  testthat::expect_error(anycensusk(codes = "11", type = "population", year = 2015))
})
