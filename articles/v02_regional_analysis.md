...
![](v02_regional_analysis_files/figure-html/choropleth_1-1.png) In the
Gyeongsangnam-do region, large metropolitan cities such as Busan and
Ulsan show population decline, while surrounding suburban areas
demonstrate notable population gains. Rural areas in the western part of
the province exhibit relatively stable population trends.
...
2.  Because tax data is not available at the *Gu* level of large cities,
    aggregate district-level (`adm2_code`) data to the 4-digit prefix
    level.
...
![](v02_regional_analysis_files/figure-html/bivariate_2-1.png)

The bivariate map highlights the strong centrality of the Seoul
...
few metropolitan centers—fall into the low population–low tax category.
...
3.  Visualize the distribution of sex ratios:
    ...
    - Overlay regional distributions (Seoul, Incheon, Gyeonggi-do) in
      color with facets.
4.  Map the sex ratio across districts, using a diverging color scale
    centered on 100 (balanced sex ratio).
...
![](v02_regional_analysis_files/figure-html/histogram_3-1.png)
...
![](v02_regional_analysis_files/figure-html/choropleth_3-1.png)

Within the Seoul Metropolitan Area, the central city districts (Seoul)
show a higher proportion of females, whereas the outer suburban areas
tend to show male-dominated populations. This highlights spatial
differences in gender distribution across the metropolitan region.

## Example 4: Socioeconomic profiling

Socioeconomic status can be profiled using multiple indicators such as
housing conditions, population characteristics, mortality rates, and
social security usage. Principal Component Analysis (PCA) is a useful
technique to reduce the dimensionality of such multivariate data and
identify key underlying factors. Here is an example of performing PCA on
various socioeconomic indicators at the basic local government level in
South Korea using `tidycensuskr`.

First, we load and combine multiple datasets from the 2020 census,
including housing, population, mortality, and social security data. We
then aggregate the data to the basic local government level (4-digit
prefix of `adm2_code`) and compute several socioeconomic indicators.
Using
[`anycensus()`](https://sigmafelix.github.io/tidycensuskr/reference/anycensus.md)
function for multiple times, we merge the datasets into a single
wide-format data frame. Variables are aggregaed appropriately (summing
counts, averaging rates) to reflect the characteristics of basic local
governments.
...
Afterwards, we perform PCA on the selected socioeconomic indicators and
visualize the results using a biplot. The biplot displays both the
districts (points) and the variables (arrows), allowing us to interpret
the relationships between different socioeconomic factors and how they
vary across regions.

For a biplot, we label the districts with their names and codes for
better identification. Looking at the rotation matrix, the first
component is moderately correlated with fertility and mortality rates
and basic living income beneficiary rate (basic living security
hereafter), and negatively correlated with persons per housing unit. The
second component is dominantly correlated with sex ratio to the negative
and basic living security rate to the positive. The biplot using the
first and second principal components is used to visualize these
relationships.
...
# Biplot with PC1 and PC3
biplot(prc_df, choices = c(1, 2))
```

![](v02_regional_analysis_files/figure-html/pca_demo_2-1.png)

Interestingly, the biplot reveals that sex ratio is independent of
crowding and mortality rate, whereas basic living security rate is
negatively correlated, as evidenced by the orientation of each
component’s vectors compared to others. Districts in Seoul are mostly
located in the upper left quadrant, indicating lower sex ratios and
higher basic living security rates, while suburban areas in Gyeonggi-do
tend to have higher sex ratios and lower basic living security rates.
This analysis provides insights into the socioeconomic disparities
across different regions within the Seoul Metropolitan Area. Inland and
border areas are generally associated with higher basic living security
rates, indicating potential socioeconomic challenges in these regions.
