...
vignette: >
  %\VignetteIndexEntry{Examples with ggplot2} 
...
```{r choropleth-1, fig.width=10, fig.height=4}
...
```{r choropleth-2, fig.width=10, fig.height=4}
adm2_sf_2020 <- load_districts(year = 2020)
...
population_2010 <- censuskor |>
  filter(year == 2010, type == "population", class2 == "total")
population_2015 <- censuskor |>
  filter(year == 2015, type == "population", class2 == "total")
population_2020 <- censuskor |>
  filter(year == 2020, type == "population", class2 == "total")
...
map_2015 <- ggplot(data = sf_2015_merged) +
  geom_sf(aes(fill = value)) +
  labs(title = "Total Population (2015)") +
  scale_fill_gradientn(colors = fill_cols) +
  theme_minimal()
...
map_2020 <- ggplot(data = sf_2020_merged) +
  geom_sf(aes(fill = value)) +
  labs(title = "Total Population (2020)") +
  scale_fill_gradientn(colors = fill_cols) +
  theme_minimal()
...
## Basemap
```{r basemap, fig.width = 6, fig.height = 6, dpi = 254}
library(tmap)
library(geodata)
library(tmaptools)
...
- Boundaries in bold represent provinces (_Si-Do_).
- The other boundaries are for districts/municipalities (_Si-Gun-Gu_).