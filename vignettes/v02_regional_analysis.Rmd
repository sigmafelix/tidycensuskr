...
vignette: >
  %\VignetteIndexEntry{Analyzing South Korean Census Data with tidycensuskr} 
...
df_2010_pop <- anycensus(year = 2010, codes = c("Gyeongsangnam-do", "Busan", "Ulsan"), type = "population")
df_2020_pop <- anycensus(year = 2020, codes = c("Gyeongsangnam-do", "Busan", "Ulsan"), type = "population")
...
korea_inset <- ggplot() +
  ...
  geom_sf(data = sf_target_boundary, fill = "grey30") +
  theme_void()
...
```{r filter-merge_2}
# Load census data
df_2020_pop <- anycensus(year = 2020, type = "population")
df_2020_tax <- anycensus(year = 2020, type = "tax")
