...
Socioeconomic status can be profiled using multiple indicators such as housing conditions, population characteristics, mortality rates, and social security usage. Principal Component Analysis (PCA) is a useful technique to reduce the dimensionality of such multivariate data and identify key underlying factors. Here is an example of performing PCA on various socioeconomic indicators at the basic local government level in South Korea using `tidycensuskr`.

First, we load and combine multiple datasets from the 2020 census, including housing, population, mortality, and social security data. We then aggregate the data to the basic local government level (4-digit prefix of `adm2_code`) and compute several socioeconomic indicators. Using `anycensus()` function for multiple times, we merge the datasets into a single wide-format data frame. Variables are aggregaed appropriately (summing counts, averaging rates) to reflect the characteristics of basic local governments.
...
Afterwards, we perform PCA on the selected socioeconomic indicators and visualize the results using a biplot. The biplot displays both the districts (points) and the variables (arrows), allowing us to interpret the relationships between different socioeconomic factors and how they vary across regions.

For a biplot, we label the districts with their names and codes for better identification. Looking at the rotation matrix, the first component is moderately correlated with fertility and mortality rates and basic living income beneficiary rate (basic living security hereafter), and negatively correlated with persons per housing unit. The second component is dominantly correlated with sex ratio to the negative and basic living security rate to the positive. The biplot using the first and second principal components is used to visualize these relationships.

```{r pca_demo_2, fig.width=7.5, fig.height=7}
# Run PCA
prc_df <-
  df_wide_re |>
  dplyr::select(3:7) |>
